...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : 3-axis BLDC motor controller - STM32F103C6 Blue Pill
  ******************************************************************************
  * Safe startup: all motor outputs default to OFF.
  * Comms: USB CDC virtual COM port (PA11/PA12) → Atomic Pi /dev/ttyACM0
  *
  * FIRST BRING-UP CHECKLIST:
  *   1. Flash via SWD (PA13/PA14). Power up with motors disconnected.
  *   2. Connect USB to Atomic Pi. On Linux: screen /dev/ttyACM0 115200
  *      (baud ignored by CDC but needed by screen)
  *   3. Send "P\n"  → expect "PONG\n"
  *   4. Send "?\n"  → all 3 motors should show enabled=0, duty=0
  *   5. Enable M0:  "E0\n"  → "OK:E0\n"
  *   6. Low duty:   "S0,200\n"  → watch LED blink rate / Hall counts
  *   7. Query:      "?\n"  → commut_count increments if motor turns
  *   8. E-stop:     "DA\n" → all motors off immediately
  *   9. Periodic STATUS lines print every 1s automatically
  ******************************************************************************
  */
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define STATUS_REPORT_INTERVAL_MS   1000U
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  /* Motor controller: all motors DISABLED at boot (safe default) */
  MC_Init(&htim1, &htim2, &htim3);

  /* Comms layer over USB CDC */
  COMMS_Init();

  /* Blink onboard LED once to confirm boot */
  ...
  while (1)
  {
    ...
    /* 2. Poll Hall sensors and update commutation counts */
    MC_CommutationTick();

    /* 3. Periodic status broadcast every 1s (only if host connected) */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3|GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_8, GPIO_PIN_RESET);
  ...
  /* Hall sensors – Motor 0 (PA3/PA4/PA5) */
  GPIO_InitStruct.Pin = GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /* Hall sensors – Motor 1 (PB10/PB11/PB12), Motor 2 spare (PB4/PB5/PB9) */
  ...
  /* Motor enable outputs: PB3 (M0_EN), PB6 (M1_EN), PB7 (M2_EN), PB8 (spare EN) */
...
void Error_Handler(void)
{
  ...
  MC_DisableAll();         /* Safe: kill all motors on any HAL fault */
  __disable_irq();
  /* Rapid LED blink = fault indicator */
  while (1) {
